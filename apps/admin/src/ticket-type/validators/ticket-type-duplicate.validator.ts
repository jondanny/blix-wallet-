...
export class TicketTypeDuplicateValidator implements ValidatorConstraintInterface {
  ...
  async validate(name: string, args: ValidationArguments): Promise<boolean> {
    ...
    if ('uuid' in args.object) {
      const { uuid, ticketProviderId, ticketDateStart, ticketDateEnd } = args.object as UpdateTicketTypeDto;
      const currentTicketType = await this.ticketTypeService.findByUuidAndTicketProvider(uuid, ticketProviderId);
